...
{fbvFormArea id="userDetails"}
	...
	{if !$disableEmailSection}
		{fbvFormSection title="about.contact"}
			...
			{fbvElement type="text" label="user.phone" id="phone" name="phone" required="true" value=$phone maxlength="24" inline=true size=$fbvStyles.size.SMALL} <!-- script yang ditambahkan -->
		{/fbvFormSection}
	{/if}
	...
	{fbvFormArea id="userFormExtendedLeft"}
		{fbvFormSection}
			...
			{if !$disablePhoneSection}
				<!-- {fbvElement type="text" label="user.phone" name="phone" id="phone" value=$phone maxlength="24" inline=true size=$fbvStyles.size.SMALL} --> <!-- script yang dihilangkan -->
			{/if}
