////////////////////////////////////////////
// WPJSON Announcement Web Child GET_OTHER
////////////////////////////////////////////

function wpjan_get_other() {
	$limit = 7; // SET UP HERE
	...
	for($i=0; $i<$items; $i++) {
		...
		if(isset($arr[$i]->title->rendered) && $x<=$limit) {
			...
			$date_converted = wpjan_get_other_konversi_tanggal("j F Y", $date_converted, $bahasa="id");
...
add_shortcode('wpjson_announcement_get_other', 'wpjan_get_other');

function wpjan_get_other_konversi_tanggal($format, $tanggal="now", $bahasa="id") {
