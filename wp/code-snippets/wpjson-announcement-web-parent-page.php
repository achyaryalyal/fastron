...
function wpjanpage() {
	...
	for($i=0; $i<$total_items; $i++) {
		...
		if(isset($arr[$i]->title->rendered) && $x<=$limit) {
			...
			$result .= '<li class="wpjanpage-list"><h3 class="font-osc"><a href="'.$link.'" target="_blank">'.$print_title.'</a></h3><p><i class="fa fa-calendar-alt"></i> '.$date_converted.' <i class="fa fa-id-card-alt p-l-6"></i> '.$subdomain.'</p></li>';
		}
	}
	$result .= '</ul>
	<style>
